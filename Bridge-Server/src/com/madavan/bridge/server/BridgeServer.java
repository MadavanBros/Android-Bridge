...
public class BridgeServer extends Thread {
	...
	public BridgeServer(int port) throws IOException {
		...
		System.out
				.println("BridgeServer@constructor: ServerSocket initialized.");
	}
	...
	public void run() {
		try {
			System.out
					.println("BridgeServer@run: ServerSocket accepting clients.");

			while (true) {
				ArrayList<BridgePlayer> players = new ArrayList<BridgePlayer>();
				for (int i = 0; i < 4; i++)
					players.add(new BridgePlayer(_serverSocket.accept()));
